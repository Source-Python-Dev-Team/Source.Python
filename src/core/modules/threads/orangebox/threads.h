...
#ifndef _THREADS_ORANGEBOX_H
#define _THREADS_ORANGEBOX_H
	...
	#include <unistd.h>
	#define TY_Unit us
	#define TY_Hook usleep
	#define TY_Sleep(us) \
		struct timespec ts = { \
			.tv_sec = (long int) (us / 1000000), \
			.tv_nsec = (long int) (us % 1000000) * 1000ul \
		}; \
		nanosleep(&ts, NULL);

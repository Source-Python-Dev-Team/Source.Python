...
#define NOTIFY_MOVEMENT_LISTENER(name, ...) \
	CALL_LISTENERS_WITH_MNGR( \
		Get##name##ListenerManager(), \
		GetEntityObject(GetGameMovement()->GetPlayerMixin()->GetThis()), \
		__VA_ARGS__ \
	);

#define NOTIFY_MOVEMENT_LISTENER_WITH_MOVEDATA(name, ...) \
	NOTIFY_MOVEMENT_LISTENER(name, object(ptr(GetGameMovement()->mv)), __VA_ARGS__)
...
#define DEFINE_GENERIC_MOVEMENT_LISTENER(name, hook, type) \
	...
	{ \
		NOTIFY_MOVEMENT_LISTENER(name); \
		return false; \
	}

#define DEFINE_GENERIC_MOVEMENT_LISTENER_WITH_MOVEDATA(name, hook, type) \
	DEFINE_MOVEMENT_LISTENER(name, hook, type) \
	{ \
		NOTIFY_MOVEMENT_LISTENER_WITH_MOVEDATA(name); \
		return false; \
	}
...
DECLARE_MOVEMENT_LISTENER(OnPlayerGroundChanged)
